...
void Rasterize_CUDA(
  ...
  for(uint32_t TriangleIndex = 0; TriangleIndex < Triangles->TriangleCount;
    ...
    WorldToCamera.multVecMatrix(Vertex0World, Vertex1Camera);
    WorldToCamera.multVecMatrix(Vertex1World, Vertex2Camera);
    WorldToCamera.multVecMatrix(Vertex2World, Vertex0Camera);
